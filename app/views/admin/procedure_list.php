<div class="col-9">
    <h1 style="text-align: center">Procedure List</h1>
        ...
        <table id="session_table" class="table">
            ...
            <?php foreach ($data['list'] as $key => $value){?>
                <tr>
                    ...
                    if(Session::get('user_role') == 'admin'){?>
                    <td>
                        <form action="<?php echo BASE_URL?>/ProcedureList/delete" method="post">
...
<script>
    ...
    function btn_update(asd) {
        ...
        $('#data_form').attr('action','http://localhost/se_addmission/ProcedureList/update');
    ...
    function btn_add(asd) {
        $('#dataModal').modal('show');
        $('#data_form').attr('action','http://localhost/se_addmission/ProcedureList/addNew');
