        ...
        <table id="session_table" class="table">
            ...
            <?php foreach ($data['list'] as $key => $value){?>
                <tr>
                    ...
                    <td>
                        <form action="<?php echo BASE_URL?>/University/delete" method="post">
            ...
            <form method="post" id="data_form" enctype="multipart/form-data">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title"><span id="modal_title">Add University</span></h4>
                        <button type="button" class="close" data-dismiss="modal">&times;</button>
                    </div>
                    <div class="modal-body">
                        <label for="session_name" id="lbl_session">Enter university name</label>
...
<script>
    ...
    function btn_update(asd) {
        ...
        $('#data_form').attr('action','http://localhost/se_addmission/University/update');
        $('#modal_title').text('Update University');
    ...
    function btn_add(asd) {
        $('#dataModal').modal('show');
        $('#data_form').attr('action','http://localhost/se_addmission/University/addNew');
        $('#modal_title').text('Add University');
        $('#lbl_session').text('Enter university name');
        $('#session_id').val("");
        $('#session_name').val("");
        $('#action').val('Add');
    };
